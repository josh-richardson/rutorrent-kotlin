...
class TorrentRequest(credentials: Credentials, urlAppend: String) : Request(credentials, urlAppend) {
    ...
    val torrentRequestData = arrayListOf(TorrentRequestProperty.HASH, TorrentRequestProperty.NAME, TorrentRequestProperty.PATH, TorrentRequestProperty.LEFT_BYTES, TorrentRequestProperty.SIZE_BYTES);
    ...
    fun getTorrents(): ArrayList<Torrent> {
        ...
        for (n in 0..nodes.length - 1) {
            if (nodes.item(n).childNodes.length == torrentRequestData.size) {
                ...
                val tor = Torrent(children.item(0).textContent, children.item(1).textContent, children.item(2).textContent, children.item(4).textContent.toLong(), children.item(3).textContent == "0")
                toReturn.add(tor)
            }
        }
        return toReturn
